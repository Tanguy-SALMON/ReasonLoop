...
4. **Save Templates**: Save the generated HTML templates to the output folder
5. **Present Options**: Compile all designs for customer selection

## TASK STRUCTURE

Create 4-5 tasks to generate email designs from brand intelligence.

Available abilities: [text-completion, email-design, website-intelligence, save-email-templates]
...
3. Use save-email-templates ability to save the HTML templates to disk
4. Use text-completion to create a summary comparing the 3 designs

## EXAMPLE OUTPUT

[{{"id": 1, "task": "Use the email-design ability to generate 3 email design variations based on the brand intelligence data. Each design should have a different persona: minimalist, bold, and elegant. Campaign goal: promote new arrivals and drive conversions.", "ability": "email-design", "dependent_task_ids": [], "status": "incomplete"}}, {{"id": 2, "task": "Save the generated HTML email templates to the output folder", "ability": "save-email-templates", "dependent_task_ids": [1], "status": "incomplete"}}, {{"id": 3, "task": "Analyze and compare the 3 generated email designs, highlighting the strengths of each approach and provide a final recommendation report with subject lines, preview text, and key design rationale", "ability": "text-completion", "dependent_task_ids": [1], "status": "incomplete"}}]
